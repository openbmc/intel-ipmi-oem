                                  ...
                                  ipmi_context_t context)
{
    ...
    *dataLen = 0; // default to 0 in case of an error
    ...
    size_t offset = (*(static_cast<uint8_t*>(request) + 2) << 8) |
                    *(static_cast<uint8_t*>(request) + 1);
    size_t readCount = *(static_cast<uint8_t*>(request) + 3);

    if (readCount > maxMessageSize - 1)
    {
        return IPMI_CC_INVALID_FIELD_REQUEST;
    }
    ipmi_ret_t status = replaceCacheFru(devId);
    ...
    size_t padByteLen = readCount - fromFRUByteLen;
