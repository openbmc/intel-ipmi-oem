...
static constexpr uint32_t defaultBeepFrequency = 2000;
...
static constexpr char beepDevName[] = "/dev/input/event0";
int mtmTestBeepFd = -1;
    ...
    switch (signalType)
    {
        ...
        case SmSignalSet::smSpeaker:
        {
            switch (action)
            {
                ...
                {
                    phosphor::logging::log<phosphor::logging::level::INFO>(
                        "case Speaker SmActionSet::forceDeasserted");
                ...
                {
                    phosphor::logging::log<phosphor::logging::level::INFO>(
                        "case Speaker SmActionSet::forceAsserted");
                    ...
                    event.type = EV_SND;
                    event.code = SND_TONE;
                    event.value = defaultBeepFrequency;

                    if (::write(mtmTestBeepFd, &event,
                ...
                {
                    phosphor::logging::log<phosphor::logging::level::INFO>(
                        "case Speaker SmActionSet::revert");
                    if (mtmTestBeepFd != -1)
                    {
                        ::close(mtmTestBeepFd);
                        mtmTestBeepFd = -1;
