                                      ...
                                      uint8_t action, uint8_t slotNum)
{
    ...
    if (getBusNumFromPath(linkPath, portNum))
    {
        auto res = ctx->bus->yield_method_call<int>(
            ctx->yield, ec, service, "/xyz/openbmc_project/mctp",
            "xyz.openbmc_project.MCTP.Base", "RemoveMCTPMuxSlot", action,
            std::stoi(portNum));
    }
    return ipmi::responseSuccess();
    ;
