                    ...
                    uint8_t instance, uint8_t actionByte)
{
    ...
    switch (signalType)
    {
        case SmSignalGet::smChassisIntrusion:
        {
            ...
            uint8_t status = 0;
            if (!intrusionStatus->compare("xyz.openbmc_project.Chassis.Intrusion.Status.Normal"))
            {
                status = static_cast<uint8_t>(IntrusionStatus::normal);
            }
            else if (!intrusionStatus->compare("xyz.openbmc_project.Chassis.Intrusion.Status.HardwareIntrusion"))
            {
                status =
                    static_cast<uint8_t>(IntrusionStatus::hardwareIntrusion);
            }
            else if (!intrusionStatus->compare("xyz.openbmc_project.Chassis.Intrusion.Status.TamperingDetected"))
            {
                status =
