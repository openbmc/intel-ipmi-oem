...
static ipmi::RspType<> startOrStopService(boost::asio::yield_context yield,
...
static ipmi::RspType<> handleMCTPFeature(boost::asio::yield_context yield,
                                         const uint8_t enable,
                                         const std::string& binding)
{
    ...
    for (const auto& objectPath : configPaths)
    {
        ...
        {
            return startOrStopService(yield, enable,
                                     ...
                                     const uint16_t reserved)
{
    ...
    switch (feature)
    {
        case ipmi::SupportedFeatureControls::mctp:
            switch (featureArg)
            {
                case ipmi::SupportedMCTPBindings::mctpPCIe:
                    return handleMCTPFeature(yield, enable, "MCTP_PCIe");
                case ipmi::SupportedMCTPBindings::mctpSMBusHSBP:
                    return handleMCTPFeature(yield, enable, "MCTP_SMBus_HSBP");
                case ipmi::SupportedMCTPBindings::mctpSMBusPCIeSlot:
                    return handleMCTPFeature(yield, enable,
        ...
        case ipmi::SupportedFeatureControls::pcieScan:
            ...
            startOrStopService(yield, enable,
