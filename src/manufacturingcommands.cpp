    ...
    for (const auto& [path, fru] : obj)
    {
        for (const auto& [intf, propMap] : fru)
        {
            if (intf == "xyz.openbmc_project.Inventory.Item.Board.Motherboard")
            {
                ...
                {
                    std::cerr << "MotherBoard FRU config data type invalid, "
                                 "not used.\n";
                    continue;
                }
                busNum = *fruBus;
...
bool readMacFromFru(ipmi::Context::ptr ctx, uint8_t dataType,
                    std::array<uint8_t, maxEthSize>& ethData)
{
    ...
    {
        std::cerr << "found mac fru bus " << fruBus << " address " << fruAddress
                  << "\n";

        macOffset += dataType * maxEthSize;
        std::vector<uint8_t> writeData;
        writeData.push_back(static_cast<uint8_t>(macOffset & 0xff));
        if (macOffset > 0xff)
        {
            writeData.push_back(static_cast<uint8_t>((macOffset >> 8) & 0xff));
        }
...
bool writeMacToFru(ipmi::Context::ptr ctx, uint8_t dataType,
                   std::array<uint8_t, maxEthSize>& ethData)
{
    uint64_t macOffset = 0xffff;
    uint64_t fruBus = 0;
                                     ...
                                     std::array<uint8_t, maxEthSize> ethData)
{
    ...
    if (writeMacToFru(ctx, dataType, ethData))
    {
        resetMtmTimer(ctx);
        return ipmi::responseSuccess();
    }
