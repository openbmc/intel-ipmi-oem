...
ipmi::RspType<> appMTMSetSignal(uint8_t signalTypeByte, uint8_t instance,
                                uint8_t actionByte,
                                std::optional<uint8_t> pwmSpeedVal)
{

    uint8_t pwmSpeed = pwmSpeedVal.value_or(0);
    ...
    switch (signalType)
    {
        ...
        {
            if ((action == SmActionSet::forceAsserted) &&
                (pwmSpeed > 100 || instance == 0))
            {
                retCode = ccInvalidFieldRequest;
                break;
            }
...
void register_mtm_commands()
{
    ...
    ipmi::registerHandler(
        ...
        ipmi::Privilege::User, ipmi::appMTMSetSignal);
