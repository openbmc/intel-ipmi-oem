                               ...
                               ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    if (regionLockUnlocked == value.get<uint8_t>())
    {
    ...
    if (requestData->sessionId != value.get<uint8_t>())
    {
                           ...
                           ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    if (requestData->offset + requestData->length > regUsedVal.get<uint16_t>())
    {
        return IPMI_CC_REQ_DATA_LEN_INVALID;
    }
    ...
    if (regionLockUnlocked != lockPolicyVal.get<uint8_t>())
    {
        return IPMI_CC_PARAMETER_NOT_SUPPORT_IN_PRESENT_STATE;
    }
...
static void register_netfn_smbios_functions(void)
{
    ...
    print_registration(NETFUN_INTEL_APP_OEM,
