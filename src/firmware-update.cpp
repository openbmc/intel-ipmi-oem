...
namespace ipmi
{
constexpr Cc ccInvalidCodeEntered = 0x80;

static inline auto responseInvalidCodeEntered()
{
    return response(ccInvalidCodeEntered);
}
    ...
    std::array<uint8_t, FW_RANDOM_NUMBER_LENGTH> &randNum)
{
    ...
    for (int i = 0; i < FW_RANDOM_NUMBER_LENGTH; i++)
    {
        if (fw_random_number[i] != randNum[i])
        {
            ...
            return ipmi::responseInvalidCodeEntered();
        }
    }
