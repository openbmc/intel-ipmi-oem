...
static constexpr uint8_t certRootKeySize = 96;
static constexpr uint8_t cskSize = 24;

ipmi::RspType<std::array<uint8_t, certRootKeySize>,
              std::optional<std::array<uint8_t, cskSize>>>
    ipmiGetFwRootCertData(uint8_t certId)
{
    ...
    std::array<uint8_t, certRootKeySize> certRootKey = {0};

    try
    {
        SPIDev spiDev(bmcPfmMTDDev);
        spiDev.spiReadData(certOffset, certRootKeySize, certRootKey.data());

        if (cskOffset)
        {
            std::array<uint8_t, cskSize> cskData = {0};
            spiDev.spiReadData(cskOffset, cskSize, cskData.data());
            return ipmi::responseSuccess(certRootKey, cskData);
        }
    }
