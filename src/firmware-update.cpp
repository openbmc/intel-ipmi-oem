...
static constexpr int maxFirmwareImageSize = 32_MB;
  ...
  public:
    ...
    enum HashCheck verify()
    {
        if (check == HashCheck::requested)
        {
            ...
            {
                if (DEBUG)
                    std::cerr << "transfer sha2 verify passed\n";
                check = HashCheck::sha2Success;
            }
            else
            {
                if (DEBUG)
                    std::cerr << "transfer sha2 verify failed\n";
...
class FwUpdateStatusCache
{
  public:
    ...
    {
        if (DEBUG)
            std::cerr << "fw-state: 0x" << std::hex << (int)fwState << '\n';
        if ((fwState == fwStateIdle || fwState == fwStateInit) &&
            local_download_is_active())
        {
            fwState = fwStateDownload;
            progressPercent = 0;
        }
        return fwState;
    }
    ...
    std::string getSoftwareObjPath()
    {
        return softwareObjPath;
    }
    void setSoftwareObjPath(std::string &objPath)
    {
        softwareObjPath = objPath;
        fwState = fwStateWrite;
        progressPercent = 0;
    ...
    uint8_t activationTimerTimeout()
    {
        ...
        if (progressPercent >= 95)
        {
            ...
            fwState = fwStateReady;
        }
        std::cerr << " progressPercent = " << (int)progressPercent << "\n";
        return progressPercent;
    }
  ...
  protected:
    void parseProperties(std::map<std::string, ipmi::DbusVariant> &properties)
    {
        if (DEBUG)
        ...
        for (const auto &t : properties)
        {
            ...
            if (key == "state")
            {
                ...
                if (state == "INIT")
                    ...
                    fwState = fwStateAcCycleRequired;
                else
                {
                    fwState = fwStateError;
            ...
            {
                progressPercent = std::get<int32_t>(value);
                if (DEBUG)
                    std::cerr << ", pct=" << (int)progressPercent;
            }
            else if (key == "msg")
            {
                msgStr = std::get<std::string>(value);
                if (DEBUG)
                    std::cerr << ", msg='" << msgStr << '\'';
            }
            else if (key == "Progress")
            {
                progressPercent = std::get<uint8_t>(value);
...
static void activateImage(const char *objPath)
{
    ...
    if (fwUpdateStatus.getDeferRestart() == false)
    {
        phosphor::logging::log<phosphor::logging::level::INFO>(
            "activating Image: ",
            phosphor::logging::entry("OBJPATH =%s", objPath));
    ...
    std::unique_ptr<sdbusplus::bus::match::match> &fwUpdateMatchSignal)
{
    ...
    auto callback = [&fwUpdateMatchSignal](sdbusplus::message::message &m) {
        if (DEBUG)
        ...
        if (DEBUG)
            std::cerr << "[complete] obj path = " << objPath.str << "\n";
        for (auto &interface : intfPropsPair)
        {
            ...
            if (interface.first == "xyz.openbmc_project.Software.Activation")
            {
                ...
                fwUpdateStatus.setSoftwareObjPath(objPath.str);
                activateImage(objPath.str.c_str());
                if (DEBUG)
...
static int transferImageFromFile(const std::string &uri, bool move = true)
{
    std::error_code ec;
    if (DEBUG)
        std::cerr << "transferImageFromFile(" << uri << ")\n";
    if (move)
    {
...
static int transferImageFromUsb(const std::string &uri)
{
    ...
    if (DEBUG)
        std::cerr << "transferImageFromUsb(" << uri << ")\n";
...
static bool transferFirmwareFromUri(const std::string &uri)
{
    ...
    if (DEBUG)
        std::cerr << "transferFirmwareFromUri(" << uri << ")\n";
...
static int getUsbStatus()
{
    static constexpr char usbGadgetBase[] = "/sys/kernel/config/usb_gadget/";
    auto usb_device = std::filesystem::path(usbGadgetBase) / fwUpdateUSBDevName;
    std::error_code ec;
...
static uint8_t getActiveBootImage(void)
{
    ...
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "Exception caught in getActiveBootImage",
    ...
    ipmiSetFirmwareUpdateMode(std::array<uint8_t, fwRandomNumLength> &randNum)
{
    ...
    if (fwUpdateStatus.state() != FwUpdateStatusCache::fwStateIdle
        // TODO: Allowing fwStateInit here to let image activation available
        // without being in fwStateIdle, need to fix/adjust the state machine
        // to match xyz.openbmc_project.Software.BMC.Updater service activation
        // mechanism at finer grain
        && fwUpdateStatus.state() != FwUpdateStatusCache::fwStateInit)
    {
        phosphor::logging::log<phosphor::logging::level::INFO>(
...
ipmi::RspType<> ipmiExitFirmwareUpdateMode()
{
    ...
    switch (fwUpdateStatus.state())
    {
        ...
        case FwUpdateStatusCache::fwStateVerify:
            break;
        case FwUpdateStatusCache::fwStateWrite:
            break;
        case FwUpdateStatusCache::fwStateReady:
...
ipmi::RspType<bool, bool, bool, bool, uint4_t> ipmiGetSetFirmwareUpdateControl(
    const uint8_t controlReq, const std::optional<uint8_t> fileNameLen,
    const std::optional<std::vector<uint8_t>> fileName)
{
    static std::string fwXferUriPath;
    bool imageTransferStarted = false;
    bool imageTransferCompleted = false;
    bool imageTransferAborted = false;
    bool usbAttached = getUsbStatus();
    ...
    switch (static_cast<FwUpdateCtrlReq>(controlReq))
    {
        ...
        case FwUpdateCtrlReq::setFirmwareFilename:
            if (fileNameLen > fwPathMaxLength)
            {
                phosphor::logging::log<phosphor::logging::level::DEBUG>(
                    "Filename length out of boundary.");
                return ipmi::responseInvalidFieldRequest();
            }
            ...
            fwXferUriPath.clear();
            fwXferUriPath.assign((*fileName).begin(), (*fileName).end());
            break;
    ...
    return ipmi::responseSuccess(imageTransferStarted, imageTransferCompleted,
                                 imageTransferAborted, usbAttached,
                                 static_cast<uint4_t>(0));
}
...

{
    ...
    uint8_t status = fwUpdateStatus.state();
    ...
    ipmiFwImageWriteData(const std::vector<uint8_t> writeData)
{
    ...
    if (fwUpdateStatus.state() != FwUpdateStatusCache::fwStateDownload)
    {
        phosphor::logging::log<phosphor::logging::level::DEBUG>(
