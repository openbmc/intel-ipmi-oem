...
static constexpr uint32_t bmcActiveImgPfmMTDDevNo = 2;
static constexpr uint32_t bmcRecoveryImgMTDDevNo = 7;

static constexpr uint32_t bmcActiveImgCertKeyOffset = 0xA0;
static constexpr uint32_t bmcRecoveryImgCertKeyOffset = 0x4A0;
static constexpr uint32_t bmcActiveImgCertCSKOffset = 0x118;
static constexpr uint32_t bmcRecoveryImgCertCSKOffset = 0x518;

ipmi::RspType<std::vector<uint8_t>> // cerificate data
    ipmiGetFwRootCertData(uint8_t certId)
{
    static constexpr uint8_t ccInvalidCertificateId = 0xCC;

    if ((certId == 0) || (certId > (uint8_t)FwGetRootCertDataTag::recoveryCSK))
    {
        return ipmi::response(ccInvalidCertificateId);
    }

    uint8_t mtdDevNo = 0;
    uint32_t certOffset = 0;
    uint32_t cskOffset = 0;
    constexpr uint8_t certRootKeySize = 96;
    constexpr uint8_t cskSize = 24;
    std::vector<uint8_t> certDataRes;
    ...
    switch (certId)
    {
        ...
        case static_cast<uint8_t>(FwGetRootCertDataTag::activeCSK):
        {
            mtdDevNo = bmcActiveImgPfmMTDDevNo;
            certOffset = bmcActiveImgCertKeyOffset;
            cskOffset = bmcActiveImgCertCSKOffset;
            break;
        }
        case static_cast<uint8_t>(FwGetRootCertDataTag::recoveryCSK):
        {
            mtdDevNo = bmcRecoveryImgMTDDevNo;
            certOffset = bmcRecoveryImgCertKeyOffset;
            cskOffset = bmcRecoveryImgCertCSKOffset;
            break;
        }
    }
    ...
    try
    {
        std::array<uint8_t, certRootKeySize> certData = {0};
        SPIDev spiDev(mtdDevNo);
        spiDev.spiReadData(certOffset, certRootKeySize, certData.data());
        std::copy_n(certData.data(), certRootKeySize,
                    std::back_inserter(certDataRes));

        if (cskOffset)
        {
            std::array<uint8_t, cskSize> cskData = {0};
            spiDev.spiReadData(cskOffset, cskSize, cskData.data());
            std::copy_n(cskData.data(), cskSize,
                        std::back_inserter(certDataRes));
        }
    }
    ...
    return ipmi::responseSuccess(certDataRes);
}
#endif
