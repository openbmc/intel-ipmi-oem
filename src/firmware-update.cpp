...
class FwUpdateStatusCache
{
  public:
    ...
    void updateActivationPercent(std::string &objPath)
    {
            ...
            *busp,
            ...
            [&](sdbusplus::message::message &msg) {
                ...
                for (const auto &t : props)
                {
                    ...
                    if (key == "Progress")
                    {
                        progressPercent = std::get<uint8_t>(t.second);
    ...
    ipmiSetFirmwareUpdateMode(std::array<uint8_t, fwRandomNumLength> &randNum)
{
    ...
    if (fwUpdateStatus.getState() != FwUpdateStatusCache::fwStateIdle
        ...
        && fwUpdateStatus.getState() != FwUpdateStatusCache::fwStateInit)
    {
        phosphor::logging::log<phosphor::logging::level::INFO>(
