...
bool checkRedfishHooks(int generatorID, PlatformEventRequest* platEvent)
{
    // Save the raw IPMI string of the generatorID
    std::string genIDStr;
    std::array<uint8_t, 1> genIdArray = {static_cast<uint8_t>(generatorID)};
    const boost::beast::span<uint8_t> genIDBytes(genIdArray);
    redfish_hooks::toHexStr(genIDBytes, genIDStr);
    // Save the raw IPMI string of the request
    std::string platEventStr;
    const boost::beast::span<uint8_t> platEventBytes(
        reinterpret_cast<uint8_t*>(platEvent), sizeof(PlatformEventRequest));
    redfish_hooks::toHexStr(platEventBytes, platEventStr);
    redfish_hooks::ipmiRaw = genIDStr + platEventStr;

    // Extract the SEL data for the hook
    redfish_hooks::SELData selData = {.generatorID = generatorID,
                                      .sensorNum = platEvent->sensorNumber,
                                      .eventType =
                                          platEvent->eventDirectionType,
                                      .offset = platEvent->data[0] & 0x0F,
                                      .eventData2 = platEvent->data[1],
                                      .eventData3 = platEvent->data[2]};
