...
static constexpr uint8_t sizeBootOption = 5;
...
static constexpr uint8_t bootSoureMask = 0x3c;
    ...
    ipmi_oem_get_efi_boot_options(uint8_t parameter, uint8_t set, uint8_t block)
{
    ...
    GetSysBootOptionsResponse* resp = (GetSysBootOptionsResponse*)respArray;
...
{
    using namespace boot_options;
    uint8_t bootOption = ((data[1] & bootSoureMask) >> 2); // boot Mode
