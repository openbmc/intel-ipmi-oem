...
bool getSwVerInfo(std::array<uint8_t, 4>& verData)
{
    ...
    if (rev.has_value())
    {
        ...
        verData[0] = revision.major;

        revision.minor = (revision.minor > 99 ? 99 : revision.minor);
        verData[1] = revision.minor % 10 + (revision.minor / 10) * 16;
    }
    ...
    try
    {
        ...
        if (std::regex_match(meString, results, pattern1))
        {
            ...
            {
                verData[2] = static_cast<uint8_t>(std::stoi(results[1]));
                verData[3] = static_cast<uint8_t>(std::stoi(results[2]));
            }
        }
    }
...
ipmi::RspType<
    std::variant<std::string, std::tuple<uint8_t, std::array<uint8_t, 4>>>>
                         ...
                         uint8_t offset)
{
    ...
    switch (OEMDevEntityType(entityType))
    {
        ...
        {
            static constexpr const size_t verLen = 4;
            std::array<uint8_t, 4> readBuf = {0xff, 0xff, 0xff, 0xff};
            if (true != getSwVerInfo(readBuf))
            {
                return ipmi::responseUnspecifiedError();
            }
            return ipmi::responseSuccess(
                std::tuple<uint8_t, std::array<uint8_t, 4>>{verLen, readBuf});
        }
        break;
