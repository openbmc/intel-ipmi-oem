                               ...
                               ipmi_data_len_t dataLen, ipmi_context_t context)
{
    ...
    try
    {
        std::string service = getService(dbus, ledIntf, identifyLEDObjPath);
        ...
        service = getService(dbus, ledIntf, statusGreenObjPath);
        variant = getDbusProperty(dbus, service, statusGreenObjPath, ledIntf,
                                  "State");
        ...
        service = getService(dbus, ledIntf, statusAmberObjPath);
        variant = getDbusProperty(dbus, service, statusAmberObjPath, ledIntf,
                                  "State");
        strState = sdbusplus::message::variant_ns::get<std::string>(variant);
        state = ledAction::actionDbusToIpmi.at(
            sdbusplus::xyz::openbmc_project::Led::server::Physical::
