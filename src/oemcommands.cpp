                                     ...
                                     ipmi_context_t context)
{
    ...
    std::vector<LedFaultConfiguration> faultLedConf;
    LedFaultConfiguration ledConf;
    ...
    for (const auto& item : valueTree)
    {
        ...
        // Only scan matched fault type, if not match,continue
        auto propertyFType = interface->second.find("FaultType");
        if (propertyFType == interface->second.end())
        {
            continue;
        }
        ...
        try
        {
            Value val = propertyFType->second;
            std::string& resFType =
                sdbusplus::message::variant_ns::get<std::string>(val);
            // not match fault type
            if (fType[sfReq->eFaultType] != resFType)
            {
                continue;
            }
            ledConf.faultType = resFType;
        }
        catch (sdbusplus::message::variant_ns::bad_variant_access& e)
        {
            phosphor::logging::log<phosphor::logging::level::ERR>(e.what());
    ...
    switch (ERemoteFaultType(sfReq->eFaultType))
    {
        ...
        case (ERemoteFaultType::faultMemory):
        {
            ...
            { // Verify the required faultType & faultGroup
                // are the same with fault led configuration
                // Only set the required fault type and fault group leds
                if ((fType[sfReq->eFaultType] == faultLedConf[i].faultType) &&
                    ((fGroup[searchGroup] == faultLedConf[i].faultGroup) ||
                     (fGroup[searchGroup1] == faultLedConf[i].faultGroup)))
                {
                    std::string device =
                        sysGpioPath + std::to_string(faultLedConf[i].phyGpio) +
                        postfixValue;
                    ...
                    if (gpioFile.good())
                    {
                        gpioFile << std::to_string(
                            (uint8_t)(bitvec5[faultLedConf[i].faultOffset]));
                    }
                    gpioFile.close();
                }
            }
