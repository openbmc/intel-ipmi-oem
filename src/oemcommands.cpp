                               ...
                               ipmi_data_len_t dataLen, ipmi_context_t context)
{
    ...
    *resp |= state << LedOffset::idLedOffset;

    if (getLEDState(dbus, ledIntf, statusGreenObjPath, state) == -1)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "oem_get_led_status: fail to get Green LED status!");
        return IPMI_CC_UNSPECIFIED_ERROR;
    }
    *resp |= state << LedOffset::statusGreenOffset;

    if (getLEDState(dbus, ledIntf, statusAmberObjPath, state) == -1)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "oem_get_led_status: fail to get Amber LED status!");
        return IPMI_CC_UNSPECIFIED_ERROR;
    }
    *resp |= state << LedOffset::statusAmberOffset;
