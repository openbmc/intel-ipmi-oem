                         ...
                         uint8_t offset)
{
    ...
    switch (OEMDevEntityType(entityType))
    {
        ...
        case OEMDevEntityType::devVer:
        {
            ...
            std::array<uint8_t, 10> readBuf = {0xff, 0xff, 0xff, 0xff, 0xff,
                                               0xff, 0xff, 0xff, 0xff, 0xff};
            // data0/1: BMC version number; data6/7: ME version number
            // the others: HSC0/1/2 version number, not avaible.
            if (true !=
                getSwVerInfo(readBuf[0], readBuf[1], readBuf[6], readBuf[7]))
            {
                return ipmi::responseUnspecifiedError();
            }
            return ipmi::responseSuccess(
                std::tuple<uint8_t, std::array<uint8_t, 10>>{verLen, readBuf});
        }
        break;
