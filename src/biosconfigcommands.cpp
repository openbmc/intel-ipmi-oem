...
static constexpr const char* BiosConfigService =
                                  ...
                                  uint8_t reserved2, uint8_t reserved3)
{
    ...
    ctx->bus->yield_method_call<std::variant<std::uint32_t>>(
        ctx->yield, ec, BiosConfigService, BiosConfigBaseMgrPath,
                   ...
                   std::chrono::microseconds timeout = ipmi::IPMI_DBUS_TIMEOUT)
{
    ...
    retValue = ctx->bus->yield_method_call<std::vector<uint32_t>>(
        ...
        "SetPayload", convertForMessage(payloadStatus),
        convertForMessage(payloadType), payloadVersion, payloadSize,
                                          ...
                                          std::vector<uint8_t> payload)
{
    ...
    switch (BIOSConfigMgr::PayloadStatus(paramSel))
    {
        case BIOSConfigMgr::PayloadStatus::StartTransfer:
        {
            ...
            if (sendSetPayload(
                    ...
                    BIOSConfigMgr::PayloadType(payloadType), 0,
        ...
        case BIOSConfigMgr::PayloadStatus::InProgress:
        {
            ...
            if (sendSetPayload(ctx, retValue,
        ...
        case BIOSConfigMgr::PayloadStatus::UserAbort:
        {
            ...
            if (!sendSetPayload(ctx, retValue,
                                BIOSConfigMgr::PayloadStatus::UserAbort,
                                BIOSConfigMgr::PayloadType(payloadType),
                                pPayloadEndTransfer->payloadReservationID, 0, 0,
                                0, 0, 0, payload))
            {
