...
static uint32_t calcChecksum32(const uint8_t* buf, uint32_t len)
{
    uint32_t sum = 0;
    ...
    switch (payloadStatusIpmiToDbus.at(paramSel))
    {
        ...
        case BIOSConfigMgr::PayloadStatus::InProgress:
        {
            ...
            uint32_t calcChecksum =
                calcChecksum32(data + 16, payload.size() - 16);
            if (calcChecksum != pPayloadInProgress->payloadCurrentChecksum)
            {
                                     ...
                                     struct PayloadInfo& resp)
{
    try
    {
        ...
        resp.payloadFilePath = std::get<uint32_t>(sessionProps["PayloadFile"]);
    }
    ...
    switch (static_cast<GetPayloadParameter>(paramSel))
    {
        ...
        case GetPayloadParameter::GetPayloadData:
        {
                ...
                payloadStatusDbusToIpmi.at(BIOSConfigMgr::PayloadStatus::Valid))
            {
                ...
                std::ifstream inFile(res.payloadFilePath,
