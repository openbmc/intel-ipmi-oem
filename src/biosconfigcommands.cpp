...
// map{attributename,struct{valuetype,displayname,description,pagePtr,
// currentvalue,defaultvalue,array{map{optionstring,optionvalue}}}}
std::map<
    std::string,
    std::tuple<uint8_t, std::string, std::string, std::string,
               std::variant<bool, uint8_t, uint32_t, uint64_t, std::string>,
               std::variant<bool, uint8_t, uint32_t, uint64_t, std::string>,
               std::map<std::string, std::variant<bool, uint8_t, uint32_t,
                                                  uint64_t, std::string>>>>
    AttributesData;
    ...
    std::chrono::microseconds timeout = ipmi::IPMI_DBUS_TIMEOUT)
{
    ...
    ec.clear();
    ctx->bus->yield_method_call<>(ctx->yield, ec, service,
                                  BiosConfigBaseMgrPath, BiosConfigIntf,
...
static void cleanUpPayloadFile(uint8_t& payloadType)
{
    ...
    std::string FilePath = "/tmp/BIOSConfig/temp" + std::to_string(payloadType);
    unlink(FilePath.c_str());
    FilePath = "/tmp/BIOSConfig/Payload" + std::to_string(payloadType);
    unlink(FilePath.c_str());
	if (BIOSConfigMgr::PayloadType::IntelXMLType0 ==
		payloadTypeIpmiToDbus.at(payloadType))
    {
        unlink("/tmp/BIOSConfig/Payload1");
    }
}
    ...
    std::chrono::microseconds timeout = ipmi::IPMI_DBUS_TIMEOUT)
{
    ...
    Value variant =
        getDbusProperty(*dbus, xyz.openbmc_project.State.OperatingSystem, /xyz/openbmc_project/state/os, xyz.openbmc_project.State.OperatingSystem.Status,
                        "OperatingSystemState");
    ...
    while (pKnobsElement != nullptr)
    {
            ...
            !menupath.empty() && !defaultvalue.empty())
        {
            ...
            AttributesData.emplace(std::make_pair(
                name, std::make_tuple(1, rootPath, dname, description, curvalue,
                                      defaultvalue, optionMap)));
        }
                                          ...
                                          std::vector<uint8_t> payload)
{
    ...
    switch (static_cast<PTState>(paramSel))
    {
        ...
        case ipmi::PTState::EndTransfer:
        {
            ...
            // TODO : generateAttributesData();

            phosphor::logging::log<phosphor::logging::level::INFO>(
                " ipmiOEMSetPayload : BaseBIOSTable Property  is set");
            // TODO sendAllAttributes(ctx);
...
ipmi::RspType<> ipmiOEMSetBIOSHashInfo(
    ipmi::Context::ptr ctx, std::array<uint8_t, maxHashSize>& pwdHash,
    std::array<uint8_t, maxSeedSize>& pwdSeed, uint8_t algoInfo)
{
    ...
    std::copy(std::begin(gNVOOBdata.mBIOSHashInfo.BIOSPwdHash),
              std::end(gNVOOBdata.mBIOSHashInfo.BIOSPwdHash),
              std::begin(pwdHash));
    std::copy(std::begin(gNVOOBdata.mBIOSHashInfo.BIOSPwdSeed),
              std::end(gNVOOBdata.mBIOSHashInfo.BIOSPwdSeed),
              std::begin(pwdSeed));
    gNVOOBdata.mBIOSHashInfo.hashAlgo = algoInfo;
    // TODO
    // Set the property in BIOS config Daemon
    flushNVOOBdata();
    return ipmi::responseSuccess();
}

ipmi::RspType<std::array<uint8_t, maxHashSize>>
    ipmiOEMGetBIOSHash(ipmi::Context::ptr ctx)
{
    std::array<uint8_t, maxHashSize> pwdHash;

    if (gNVOOBdata.mIsUserPwdInitDone)
    {
        // TODO
        // Read from BIOS config Daemon
        std::copy(std::begin(pwdHash), std::end(pwdHash),
                  std::begin(gNVOOBdata.mUserPwdHash));
        return ipmi::responseSuccess(pwdHash);
    }
    else
    {
        return ipmi::response(ipmi::ipmiCCBIOSPasswordInitNotDone);
    }
}
