...
{
    std::string FilePath = "/var/oob/nvoobdata.dat";
...
static int generateAttributesData()
{
    ...
    while (pKnobsElement != nullptr)
    {
        ...
        description = pKnobsElement->Attribute("description")
                          ...
                          : "";
        phosphor::logging::log<phosphor::logging::level::ERR>(name.c_str());
                                  ...
                                  uint8_t reserved2, uint8_t reserved3)
{
    ...
    if (OSState != "OperatingState")
    {
        ...
        FILE* fp = fopen("/tmp/Error.log", "a+");
        fprintf(fp, "ipmiOEMSetBIOSCap: Before Flushing..\n");
        fclose(fp);
