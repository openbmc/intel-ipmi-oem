                      ...
                      uint8_t bytesToRead)
{
    ...
    // TODO(): This seems questionable, should this be -128 and +127 instead?
    double max = 128;
    double min = -127;
    ...
    uint8_t mMsb = (mValue >> 8) & 0xFF;
    uint8_t mBitSign = mMsb & 0x80;
    uint8_t mBitNine = mMsb & 0x01;

    // move the smallest bit of the MSB into place (bit 9)
    // the MSbs are bits 7:8 in m_msb_and_tolerance
    record.body.m_msb_and_tolerance = mBitSign | (mBitNine << 6);

    record.body.b_lsb = bValue & 0xFF;

    uint8_t bMsb = (bValue >> 8) & 0xFF;
    uint8_t bBitSign = bMsb & 0x80;
    uint8_t bBitNine = bMsb & 0x01;

    // move the smallest bit of the MSB into place (bit 9)
    // the MSbs are bits 7:8 in b_msb_and_accuracy_lsb
    record.body.b_msb_and_accuracy_lsb = bBitSign | (bBitNine << 6);

    uint8_t rExpSign = rExp & 0x80;
    uint8_t rExpBits = rExp & 0x07;
    uint8_t bExpSign = bExp & 0x80;
    uint8_t bExpBits = bExp & 0x07;

    // move rExp and bExp into place
    record.body.r_b_exponents =
        rExpSign | (rExpBits << 4) | (bExpSign >> 4) | bExpBits;

    // Set the analog reading byte interpretation accordingly
    record.body.sensor_units_1 = bSigned ? 0x80 : 0x00;
