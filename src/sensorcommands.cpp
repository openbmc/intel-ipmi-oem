    ...
    ipmiSenGetSensorEventStatus(uint8_t sensorNum)
{
    ...
    constexpr uint8_t assertionsMask = 0x08;
    std::bitset<16> assertions = static_cast<std::bitset<16>>(
        assertionsLsb | (assertionsMsb << assertionsMask));
    std::bitset<16> deassertions = static_cast<std::bitset<16>>(
        deassertionsLsb | (deassertionsMsb << assertionsMask));

    return ipmi::responseSuccess(enabled, assertions, deassertions);
}
