        ...
        (criticalInterface != sensorMap.end()))
    {
        ...
        if (warningInterface != sensorMap.end())
        {
            ...
            if (warningHigh != warningMap.end())
            {
                msgReply->readable |=
                    1 << static_cast<int>(
                        IpmiThresholdRespBits::upperNonCritical);
            ...
            if (warningLow != warningMap.end())
            {
                msgReply->readable |=
                    1 << static_cast<int>(
                        IpmiThresholdRespBits::lowerNonCritical);
                double value =
        ...
        if (criticalInterface != sensorMap.end())
        {
            ...
            {
                msgReply->readable |=
                    1 << static_cast<int>(IpmiThresholdRespBits::upperCritical);
                double value = apply_visitor(VariantToDoubleVisitor(),
                                             criticalHigh->second);
            ...
            {
                msgReply->readable |=
                    1 << static_cast<int>(IpmiThresholdRespBits::lowerCritical);
                double value = apply_visitor(VariantToDoubleVisitor(),
                                             criticalLow->second);
