                                 ...
                                 uint16_t recordID)
{
    ...
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "getVRSensorDataRecord: getSensorConnection error");
        return GENERAL_ERROR;
    }
    SensorMap sensorMap;
    if (!getSensorMap(ctx->yield, connection, path, sensorMap,
    ...
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "getVRSensorDataRecord: getSensorMap error");
        return GENERAL_ERROR;
    }
                      ...
                      uint16_t recordID, uint8_t offset, uint8_t bytesToRead)
{
    ...
    if (sensorObject != sensorMap.end())
    {
        ...
        get_sdr::SensorDataEventRecord* hdr =
            reinterpret_cast<get_sdr::SensorDataEventRecord*>(record.data());
        if (!hdr)
        {
            phosphor::logging::log<phosphor::logging::level::ERR>(
                "ipmiStorageGetSDR: record header is null");
        ...
        size_t sdrLength =
            sizeof(get_sdr::SensorDataEventRecord) + hdr->record_length;
        if (sdrLength < (offset + bytesToRead))
        {
            bytesToRead = sdrLength - offset;
        }
