                             ...
                             ipmi_data_len_t dataLen, ipmi_context_t context)
{
    ...
    *dataLen = 0; // default to 0 in case of an error
    ...
    uint16_t nextRecord = lastRecord > (req->recordID + 1) ? req->recordID + 1 : 0XFFFF;
    ...
    if (req->recordID >= sensorTree.size())
    {
        ...
        *dataLen = req->bytesToRead + 2; // next record
        std::memcpy(&resp->record_data, (char *)&data + req->offset, req->bytesToRead);
        return IPMI_CC_OK;
    }
    ...
    if (sizeof(get_sdr::SensorDataFullRecord) < (req->offset + req->bytesToRead))
    {
        req->bytesToRead = sizeof(get_sdr::SensorDataFullRecord) - req->offset;
    }

    *dataLen = 2 + req->bytesToRead; // bytesToRead + MSB and LSB of next record id

    std::memcpy(&resp->record_data, (char *)&record + req->offset, req->bytesToRead);
