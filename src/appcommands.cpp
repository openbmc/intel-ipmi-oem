...
std::optional<MetaRevision> convertIntelVersion(std::string& s)
{
    ...
    auto logAndReturnRev =
        [&](const std::string& platform, int majIdx, int minIdx, int buildIdx,
            int hashIdx, int metaIdx,
            std::optional<uint8_t> majorOverride =
                std::nullopt) -> std::optional<MetaRevision> {
    ...
    ipmi::Context::ptr& ctx)
{
    ...
    static constexpr auto uuidSubtree = "/xyz/openbmc_project/control/host0";
    static constexpr auto uuidMatch = "systemGUID";
    // Get the DBus object implementing the UUID interface
    ipmi::DbusObjectInfo objectInfo{};
    boost::system::error_code ec = ipmi::getDbusObject(
        ctx, uuidInterface, uuidSubtree, uuidMatch, objectInfo);
