...
{
    const std::string* configurationInterface = nullptr;

    for (const auto& [service, interfaces] : sensorInterfacesResponse)
    {
        for (const auto& entry : interfaces)
        {
            // Ok, so if it's a fan path we need:
            // xyz.openbmc_project.Configuration.NuvotonFan. Otherwise we need:
            // xyz.openbmc_project.Configuration.TMP175
            if (entry == "xyz.openbmc_project.Configuration.NuvotonFan")
            {
                configurationInterface = &entry;
                break;
            }
        ...
        {
            break; // stop searching interfaces.
        }
    }
