...
struct SetFaultReq
{
    ...
    uint8_t LEDState[6]; // DIMM/FAN LED state(48 bit field, LSByte first)
};
#pragma pack(pop)
struct LedFaultConfiguration
{
    std::string name;
    std::string faultGroup;
    std::string faultType;
    uint32_t phyGpio;
    uint32_t faultOffset;

    bool operator<(const LedFaultConfiguration& sname) const
    {
        return sname.name < name;
    }
};
